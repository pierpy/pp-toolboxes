function [ output ] = compute_mstate_parameters_for_prestimulus( confstruct, eeg, maps)
...
%     freq_state = cell(size(maps,1), size(1: confstruct.tf : size(eeg,1),2));
    ...
    for i = 1: confstruct.tf : size(eeg,1)
        ...
        % gfp peak
        [gfp_peaks_indices, gfp_curve] = compute_gfp_peaks(gfp_curve, confstruct.use_gfp_peaks);
        if (confstruct.debug)
...
%                       single_gfpm_corr(m) = r(1,2);
                      single_exp_var(m) = (r(1,2)*std(peak_tf))^2 / std(peak_tf)^2;
                elseif (strcmp(confstruct.similarity_measure, 'dissimilarity'))
                    single_gfpm_corr(m) = diss(maps(m,:),peak_tf);
                end
            end
            ...
            single_epoch_var(p,:)=single_exp_var;
        end
        ...
        while k <= length(start_state)-1
            ...
            dur = (stop - beg);         
            if confstruct.use_gfp_peaks == 1 && dur > confstruct.segment_min_len  
                state = ms_state(k);
...
%                 dur_state{state, ep_idx} = [dur_state{state, ep_idx}, dur*(1000/confstruct.fs)];
            ...
            elseif confstruct.use_gfp_peaks == 0
                state = ms_state(k);
                mean_gfp = mean(gfp_curve(beg:stop));
%                 dur_state{state, ep_idx} = [dur_state{state, ep_idx}, dur*(1000/confstruct.fs)];
                dur_state{state, ep_idx} = [dur_state{state, ep_idx}, dur];
...
%                 freq_state(mapnr, ep_idx) = (length(dur_state{mapnr, ep_idx})*confstruct.fs)/confstruct.tf;
    ...
    output.var_ms = exp_var;
    output.var_all = exp_var_all;
    output.cov = time_cov;
    output.nr_of_gfp_points = nr_of_gfp;
    output.mean_gfp = total_mean_of_gfp;
    output.clust=IND;
    output.gfp_correlation = single_epoch_corr;
    output.single_epoch_var = single_epoch_var;
