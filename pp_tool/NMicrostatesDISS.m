function[b_model, b_ind, b_loading, exp_var] = NMicrostatesDISS(eeg, n_mod, pmode, reruns)
...
% input arguments
    ...
    % pmode = polarity mode: 0 polarity indipendent; 1 polarity dependent
...
%             covm    = eeg * model';	% Get the unsigned covariance matrix
            for k = 1 : n_mod
                for i = 1: size(eeg,1)
                    dissm(i,k)=globalDissimilarity(eeg(i,:), model(k,:));
                end
            end
%         else
%             covm    = abs(eeg * model');						% Get the unsigned covariance matrix
%         end
         [c,ind] =  min(dissm,[],2);				            % Look for the best fit
