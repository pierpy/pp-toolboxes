...
%     eeg = avgref(eeg, self.confstruct.nch);
    dur_state = cell(size(maps,1), size(1: self.confstruct.tf : size(eeg,1),2));
    gfp_state = cell(size(maps,1), size(1: self.confstruct.tf : size(eeg,1),2));
%     freq_state = cell(size(maps,1), size(1: self.confstruct.tf : size(eeg,1),2));
    freq_state = [];
    mean_dur = [];
    time_cov = [];
    mean_gfp = [];
    for i = 1: self.confstruct.tf : size(eeg,1)
        if i+self.confstruct.tf-1 > length(eeg)
            epoch = eeg(i:end,:);
            ep_idx = round(i/self.confstruct.tf)+1;
        else
            ep_idx = round(i/self.confstruct.tf)+1;
            epoch = eeg(i:(i+self.confstruct.tf-1), :);
        end
        
        gfp_curve = compute_gfp(epoch, self.confstruct.method_GFPeak);
        ...
        % set  gfp to 1 maps
        gfp_curve_maps = compute_gfp(maps, self.confstruct.method_GFPeak);
        maps = set_gfp_all_1(maps, gfp_curve_maps);
        if (self.confstruct.debug)
            disp(strcat('epoch ', num2str(ep_idx), ' gfp set to 1'))
        end
        % gfp peak
        [gfp_peaks_indices, gfp_peaks_values, gfp_curve] = compute_gfp_peaks(gfp_curve, self.confstruct.use_gfp_peaks, self.confstruct.maxima_method, self.confstruct.minDistance, self.confstruct.minAmplitude);
        if (self.confstruct.showpeaks)
        ...
        for p = 1:size(gfp_peaks_indices, 2)
            ...
            peak_tf = epoch(peak,:);
            for m = 1: size(maps,1)
                if (strcmp(self.confstruct.similarity_measure, 'correlation'))
...
%                     single_gfpm_pcorr(m) = abs(pr(1,2));
                elseif (strcmp(self.confstruct.similarity_measure, 'dissimilarity'))
...
%         [~,INDp] = max(single_epoch_pcorr,[],2);
        ...
        while k <= length(start_state)-1
            ...
            if dur > self.confstruct.segment_min_len             
                mean_gfp = mean(gfp_curve(beg:stop));
                dur_state{state, ep_idx} = [dur_state{state, ep_idx}, dur*(1000/self.confstruct.fs)];
...
%                 freq_state(mapnr, ep_idx) = length(dur_state{mapnr, ep_idx})*(self.confstruct.tf*(1000/self.confstruct.fs/dur_sum)/(self.confstruct.tf/self.confstruct.fs));
%                 freq_state(mapnr, ep_idx) = (length(dur_state{mapnr, ep_idx})*self.confstruct.fs)/self.confstruct.tf;
