...
set(h,'Units',units,'XTickLabel',[],'YTickLabel',[],'FontSize',1);
...
uimenu(hcmenu, 'Label', 'Plot map', 'Callback', {@PlotAMap,false,0},'UserData',ud);
uimenu(hcmenu, 'Label', '3D Map', 'Callback', {@ThreeDMap,0},'UserData',ud);

if vararginmatch(varargin,'tValue')
    uimenu(hcmenu, 'Label', 'Plot t-map', 'Callback', {@PlotAMap,false,1},'UserData',ud);
...
PlotAMap(obj,eventdata,true,DoT)


function PlotAMap(obj,eventdata,ThreD,DoT)
...
subplot('Position',[0.1 0.3 0.8 0.8]);
