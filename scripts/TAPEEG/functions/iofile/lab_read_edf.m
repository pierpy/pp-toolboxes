...
hdr.intro = setstr(fread(fp,256,'uchar')'); %#ok<FPARK>
hdr.length = str2num(hdr.intro(185:192)); %#ok<ST2NM>
hdr.records = str2num(hdr.intro(237:244)); %#ok<ST2NM>
hdr.duration = str2num(hdr.intro(245:252)); %#ok<ST2NM>
hdr.channels = str2num(hdr.intro(253:256)); %#ok<ST2NM>
hdr.channelname = setstr(fread(fp,[16,hdr.channels],'char')'); %#ok<FPARK>
hdr.transducer = setstr(fread(fp,[80,hdr.channels],'char')'); %#ok<FPARK>
hdr.physdime = setstr(fread(fp,[8,hdr.channels],'char')'); %#ok<FPARK>
hdr.physmin = str2num(setstr(fread(fp,[8,hdr.channels],'char')')); %#ok<FPARK,ST2NM>
hdr.physmax = str2num(setstr(fread(fp,[8,hdr.channels],'char')')); %#ok<FPARK,ST2NM>
hdr.digimin = str2num(setstr(fread(fp,[8,hdr.channels],'char')')); %#ok<FPARK,ST2NM>
hdr.digimax = str2num(setstr(fread(fp,[8,hdr.channels],'char')')); %#ok<FPARK,ST2NM>
hdr.prefilt = setstr(fread(fp,[80,hdr.channels],'char')'); %#ok<FPARK>
hdr.numbersperrecord = str2num(setstr(fread(fp,[8,hdr.channels],'char')')); %#ok<FPARK,ST2NM>
...
try %#ok<TRYNC>
    ...
    header.hour = str2num(hdr.intro(177:178)); %#ok<ST2NM>
    header.minute = str2num(hdr.intro(180:181)); %#ok<ST2NM>
    header.second = str2num(hdr.intro(183:184)); %#ok<ST2NM>
    header.ID = tmp{3,1};
...
while strcmp(hdr.channelname(m,1:15),'EDF Annotations')
    ...
    for i = 1:hdr.records
        ...
        eventsall(i,eventsall(i,:) == 20) = 32; %#ok<AGROW>
        eventsall(i,eventsall(i,:) == 21) = 32; %#ok<AGROW>
        if numberstops > 0
            tmp = textscan(native2unicode(eventsall(i,1:numberstops(1))),'%s');
            tmp = tmp{1,1};
            time_stamp(i) = str2double(tmp(1,1))*header.samplingrate + 1; %#ok<AGROW>
        end
