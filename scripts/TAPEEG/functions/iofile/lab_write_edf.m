% lab_write_edf(filename,data,header,mode)
%
% Original Code:
% Stefan Klanke 2010
%
...
% mode 'old' or 'new' (default = 'old')

function lab_write_edf(filename,data,header,mode)
...
if strcmp(mode,'new')
    % Scale data (individual scale per channel)
...
if ~isfield(header.subject,'year')
    ...
    tmp = {'Jan';'Feb';'Mar';'Apr';'May';'Jun';'Jul'; ...
        'Aug';'Sep';'Oct';'Nov';'Dec'};
    if isnumeric(header.subject.month) & header.subject.month >= 1 & header.subject.month <= 12
...
if length(header.subject.ID) > 32
    header.subject.ID = header.subject.ID(1:32);
end
if length(header.subject.name) > 32
    header.subject.name = header.subject.name(1:32);
end
...
if ~strcmp(filename(end-4),'~')
    ...
    if isfield(header,'IFREQ') & ~isempty(header.IFREQ)
        ...
        IFREQ_file = [filename(1:end-4) '.ifreq'];
