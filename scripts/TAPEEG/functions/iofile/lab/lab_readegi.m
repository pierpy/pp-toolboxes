...
%
% Adapted for TAPEEG, F. Hatz 2012

function  [head, TrialData, EventData, SegmentCatIndex] = lab_readegi(filename,dataChunks,forceversion)
...
if exist('forceversion')
   head = lab_readegihdr(fid,forceversion);
else
   head = lab_readegihdr(fid);
end
...
if (segmented)
    ...
    if isempty(desiredFrames),
       desiredFrames = 1:head.samples;
        ...
        desiredFrames = desiredFrames(desiredFrames<=head.samples);
    end
    ...
    if (segmented),
        for i=1:head.segments,
            ...
            if ismember(i,desiredSegments)
                ...
                j=j+1;
                SegmentCatIndex(j) = segcatind; %#ok<AGROW>
                SegmentStartTime(j) = segtime; %#ok<NASGU>
