...
%% Modifications (added powerarray option):
%% July 2011, Florian Hatz Neurology Basel
%%
...
%%
%%  [spectra,Pxx_ci,freq,Lxx] = pwelch(x,window,overlap,Nfft,Fs,conf,SD
%%                                 range,plot_type,detrend,sloppy)
%%  [spectra,Pxx_ci,freq,Lxx] = pwelch(x,y,window,overlap,Nfft,Fs,conf,SD
%%                                 range,plot_type,detrend,sloppy,results)
...
%%             %%       nothing else.
%%
%% SD          %% [real scalar] Max Standard deviation of segments
%%             %%       (only powerarray)
%%             %%       Results in Lxx (vector)
%%
...
%%           %% 'powerarray' calculate power spectral density of 'x'
%%           %%              (array, all other 'result-options' omitted)
...
function [varargout] = lab_pwelch(x,varargin)
  %%
  %% COMPATIBILITY LEVEL
...
elseif ( isempty(x) || ~isvector(x) )
  ...
  arg_posn = [1 2 3 4 5 6;  %% native
              3 4 1 2 5 6;  %% Matlab R11- pwelch
              1 2 3 4 0 5;  %% Matlab R12+ pwelch
              1 2 3 4 5 6]; %% spectrum.welch defaults
...
%%  DECODE AND CHECK OPTIONAL ARGUMENTS
  ...
  for iarg = 1+arg2_is_y:nvarargin
    ...
    if ( ischar(arg) )
      ...
      elseif ( strcmp(arg,'no-strip') || strcmp(arg,'none') )
        rm_mean = 0;
      elseif ( strcmp(arg, 'powerarray' ) )
        if ( ~do_pwarray )
          n_results = 1;
          do_pwarray = n_results;
          do_power = n_results;
        end
      else
        if ( strcmp(arg, 'power' ) )
    ...
    %%  -- 'SD' arg for valid control (powerarray)
    elseif ( arg_posn(6) && iarg == arg_posn(6) )
      if ( isempty(arg) )
        x_SD = 1.5;
      elseif ( ~isscalar(arg) || ~isreal(arg) || arg < 0.0 || arg >= 5.0 )
        error( 'pwelch: arg %d (SD) must be real scalar, >=0, <5',iarg+1 );
      else
        x_SD = arg;
      end
    %%
  ...
  need_Pxx = do_pwarray || do_power || do_trans || do_coher;
  ...
  elseif ( overlap >= seg_len )
    error( 'pwelch: angle(overlap=%d) too big. Must be <length(window)=%d',...
           overlap, seg_len );
  end
  %%
    ...
    if ( need_Pxx || need_Pxy )
      if do_pwarray > 0
        ...
        for start_seg = [1:seg_len-overlap:x_len-seg_len+1]
           ...
           x_amp(i) = max(x(start_seg:end_seg)) - min(x(start_seg:end_seg));
           x_var(i) = var(x(start_seg:end_seg));
           i = i + 1;
        end
        x_amp = mean(x_amp) + x_SD*std(x_amp);
        x_var = mean(x_var) + x_SD*std(x_var);
      else
  ...
  for start_seg = [1:seg_len-overlap:x_len-seg_len+1]
    ...
    if ( need_Pxx )
      ...
      if do_pwarray > 0
        ...
        if exist('x_maxSD');
          if ((std(xx) >= x_maxSD) || (var(xx) >= x_var) || ((max(xx) - min(xx)) >= x_amp));
             Lxx(:,n_ffts) = 0;
          else
             Lxx(:,n_ffts) = 1;
             if ( conf>0 )
  ...
  if ( conf>0 && need_Pxx )
    if ( n_ffts<2 )
      ...
      Vxx(:,2) = (erfinv(conf)*sqrt(2*n_ffts/(n_ffts-1))) * sqrt(Vxx(:,2)-Pxx(:,2).^2/n_ffts);
    end
  end
  %%
  ...
  if ( range == 0 )
    if ( ~ rem(Nfft,2) )    %% one-sided, Nfft is even
      ...
      if ( need_Pxx )
        if do_pwarray > 0
          Pxx = Pxx(1:psd_len,:) + [zeros(1,n_ffts); Pxx(Nfft:-1:psd_len+1,:); zeros(1,n_ffts)];
          if ( conf>0 )
            Vxx = Vxx(1:psd_len,:) + [zeros(1,2); Vxx(Nfft:-1:psd_len+1,:); zeros(1,2)];
          end
        else
      ...
      if ( need_Pxx )
        if do_pwarray > 0
          Pxx = Pxx(1:psd_len,:) + [zeros(1,n_ffts); Pxx(Nfft:-1:psd_len+1,:)];
          if ( conf>0 )
            Vxx = Vxx(1:psd_len,:) + [zeros(1,2); Vxx(Nfft:-1:psd_len+1,:)];
          end
        else
  ...
  if do_pwarray > 0
    scale = seg_len * Fs * win_meansq;
    spectra = Pxx / scale;
    if ( conf>0 )
      Vxx = Vxx / ( n_ffts * scale );
    end
  else
  ...
  if ( nargout>=(3+(conf>0)) )
    %% frequency is 3rd or 4th return value,
    %% depends on if 2nd is confidence interval
    varargout{3+(conf>0)} = Lxx;
  end
  if ( nargout>=1 )
    varargout{1} = spectra;
  else
