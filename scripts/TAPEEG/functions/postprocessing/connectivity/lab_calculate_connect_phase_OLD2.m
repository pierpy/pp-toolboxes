...
for Nfreq = 1:size(cfg.CONNECT.freqs,1)
    ...
    if isfield(cfg.CONNECT,'EPOCH') & ~isempty(cfg.CONNECT.EPOCH) & stepmax > 1
        if isempty(Epochs) | size(Epochs.data,2) ~= step
            ...
            [Epochs,cfg.CONNECT.EPOCH,headertmp] = lab_select_epochs(data,headertmp,cfg.CONNECT.EPOCH);
        ...
        for nchan = 1:Nchans
            ...
            for i = 1:length(cfg.CONNECT.measure)
                ...
                switch measure
                    ...
                    case 'rPTE'
                        for nchan2 = 1:Nchans
                            phasetmp2 = cat(1,phasetmp(nchan,1+rdelta:end),phasetmp(nchan,1:end-rdelta),phasetmp(nchan2,1+rdelta:end),phasetmp(nchan2,1:end-rdelta));
                            ...
                            C3 = cat(1,phasetmp2(2,:),phasetmp2(3,:),phasetmp2(4,:));
                            ...
                            rpte_ci(nchan2,nchan,nstep) = 0.5*(-C1 - D4 + C3 + D1);
                        end
                end
            end
        end
