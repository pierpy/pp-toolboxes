function [settings,skipprocessing] = lab_set_xls2plot(settings,data,result,header)

skipprocessing = 0;

vars = {};
for i = 1:length(header.vars)
    vars{i} = regexprep(header.vars{i},'_',' '); %#ok<AGROW>
end
varsR = vars;
for i = 1:length(header.result)
    varsR{end+1} = regexprep(header.result{i},'_',' '); %#ok<AGROW>
end
Filename = regexprep(header.file,'_',' ');

if ~exist('settings','var') | ~isfield(settings,'xvar')
    settings.title = Filename;
    settings.mode = 'Boxplot';
    settings.xvar = length(varsR);
    settings.xlog = false;
    settings.yvar = 1;
...
Formats(end+1,1).type = 'list';
Formats(end,1).style = 'popupmenu';
Formats(end,1).items = cat(1,varsR(:),cellstr('-VarNames-'));
...
if isempty(settings) | Cancelled == 1
    ...
    function statistics = set_statistics(settings)
        if settings.xvar > 0
            if strcmp(settings.mode,'Boxplot')
                groupvalid = 1:size(result,2);
                groupvalid = setdiff(groupvalid,find(isnan(result(settings.xvar,:))));
                groupvalid = setdiff(groupvalid,find(isempty(result(settings.xvar,:))));
                ...
                tmp = unique(result(settings.xvar,varvalid));
                tmp = sort(tmp);
                if length(tmp) == 2
    ...
    function list = xvar2list(settings)
        if settings.xvar <= size(result,1)
            if strcmp(settings.mode,'Boxplot')
                groupvalid = 1:size(result,2);
                groupvalid = setdiff(groupvalid,find(isnan(result(settings.xvar,:))));
                groupvalid = setdiff(groupvalid,find(isempty(result(settings.xvar,:))));
