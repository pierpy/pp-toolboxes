function [settings,skipprocessing] = lab_set_evaluate_spectras(settings)
...
Formats(end+1,1).type = 'check';
Formats(end,1).callback = {@set_limits,'Limits','Limits','@ALL'};
...
function Limits = set_limits(Limits,settings)
  if ~isfield(settings.Spect,'SpectAll')
      Limits = [];
      return
  end
  ...
  Formats.format = 'float';
  Formats.limits = [0 inf];
  if ~isfield(Limits,'y1') | isempty(Limits.y1)
      if isempty(settings.mappingBA)
          settings.mappingBA = 1:size(settings.Spect.SpectAll,1);
      end
      if strcmp(settings.source,'mean')
          tmp = settings.Spect.SpectAllMean(settings.mappingBA,:);
      elseif strcmp(settings.source,'median')
          tmp = settings.Spect.SpectAllMedian(settings.mappingBA,:);
      else
          tmp = settings.Spect.SpectAll(:,:,settings.mappingBA);
      end
      Limits.y1 = round(min(tmp(:))*100)/100;
      Limits.y2 = round(max(tmp(:))*100)/100;
  end
  [Limits,Cancelled] = inputsdlg(Prompt,'Channels',[Formats;Formats],Limits);
  if Cancelled == 1
      Limits = [];
  end
