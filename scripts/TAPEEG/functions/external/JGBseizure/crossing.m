...
% first look for exact zeros, laatste 5 punten niet meenemen ivm extra
% conditie
ind0 = find( S(1:end-5) == 0 ); 

% then look for zero crossings between data points
S1 = S(1:end-1) .* S(2:end);
ind1 = find( S1(1:end-5) < 0 );

% bring exact zeros and "in-between" zeros together 
ind = sort([ind0 ind1]);

%extra conditie: x(k)<level en x(k+5)> level
ind = ind((S(ind+5))>level); 
