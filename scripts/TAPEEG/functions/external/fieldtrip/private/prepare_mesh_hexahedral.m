...
if ~isfield(cfg,'shift') | isempty(cfg.shift)
  warning('Using standard shift 0.3')
  cfg.shift = 0.3;
end
...
if ~ispc
    ...
    fprintf(efid,[executable ' -in ' MRfile ' -out ' meshfile ' -min ' num2str(cfg.resolution) ' -max ' num2str(cfg.resolution) ' -elem cube -material ' materialsfile ' -smooth shift -shift ' num2str(cfg.shift) '\n']);
    fclose(efid);
    dos(sprintf('chmod +x %s', shfile));
    % execute shell file
    system(shfile);
else
   tmp = find(strfind(executable,filesep),1,'last');
   setenv('PATH',[getenv('PATH') ';' executable(1:tmp)]);
   clearvars tmp
   dos([executable ' -in ' MRfile ' -out ' meshfile ' -min ' num2str(cfg.resolution) ' -max ' num2str(cfg.resolution) ' -elem cube -material ' materialsfile ' -smooth shift -shift ' num2str(cfg.shift)]); 
end
...
if ispc
    ...
    [mesh.pnt,mesh.hex,labels] = lab_read_vista_mesh(meshfile);
else
