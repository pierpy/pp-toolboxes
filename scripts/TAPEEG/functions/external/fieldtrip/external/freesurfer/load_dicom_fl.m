...
% Correct for not matching dicom-image at end (e.g. overview)
ndim1 = dcminfo0(1).Columns;
ndim2 = dcminfo0(1).Rows;
while dcminfo0(2).Columns ~= ndim1 | dcminfo0(2).Rows ~= ndim2
    dcminfo0 = dcminfo0(2:end);
    nfiles = nfiles - 1;
    flist = flist(2:end);
end
while dcminfo0(end).Columns ~= ndim1 | dcminfo0(end).Rows ~= ndim2
    dcminfo0 = dcminfo0(1:end-1);
    nfiles = nfiles - 1;
    flist = flist(1:nfiles);
end
...
function dcminfo2 = sort_by_sliceloc(dcminfo)
  ...
  if ~isfield(dcminfo,'SliceLocation') & isfield(dcminfo,'ImagePositionPatient')
      for i = 1:length(dcminfo)
          dcminfo(i).SliceLocation = dcminfo(i).ImagePositionPatient(3);
      end
  end
  for n = 1:nslices
