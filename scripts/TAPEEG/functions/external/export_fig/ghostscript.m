...
if ispc
    tmp = computer;
    if strcmp(tmp,'PCWIN64')
        path_ = fullfile(fullfile(Main_Path,'Ghostscript'),['ghostscript-9.14-windows-x64' filesep 'bin' filesep 'gswin64c.exe']);
    else
        path_ = fullfile(fullfile(Main_Path,'Ghostscript'),['ghostscript-9.14-windows-x32' filesep 'bin' filesep 'gswin32c.exe']);
    end
    clearvars tmp
    if check_store_gs_path(path_)
        return
    ...
    path_ = fullfile(fullfile(Main_Path,'Ghostscript'),['ghostscript-9.14-linux-x86_64' filesep 'gs-914-linux_x86_64']);
    if check_store_gs_path(path_)
        return
    end
