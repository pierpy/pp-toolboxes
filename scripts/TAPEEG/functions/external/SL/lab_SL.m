...
function [RESULT1,RESULT2] = lab_SL(S_in,lag,m,w1,w2,pref,speed,option,StoreHits)
 ...
 if nargin == 2 & isfield(lag,'samplingrate')
    ...
    pause(0.2);
    pref = settings.SL.pref;
    speed = floor(header.samplingrate / max(settings.SL.freqs));
    lag = round(header.samplingrate / (3*max(settings.SL.freqs)));
    m = round(3 * (max(settings.SL.freqs) / min(settings.SL.freqs)))+1;
    w1 = 2 * lag * (m - 1);
    w2 = size(S_in,1);
    option = ' ';
    StoreHits = settings.SL.storehits;
    clearvars header tmp
end
...
for i = 1:size(S_in,2)
    if max(S_in(:,i)) == 0 & min(S_in(:,i)) == 0
        BadFlag(i) = false; %#ok<AGROW>
    elseif max(isnan(S_in(:,i)))
        BadFlag(i) = false; %#ok<AGROW>
    else
        BadFlag(i) = true; %#ok<AGROW>
    end
end
BadChans = find(BadFlag == 0);
clearvars BadFlag;
...
%     end
    ...
    elseif min(Vend) < min(Vstart)
        Vstart = [1 Vstart]; %#ok<AGROW>
    end
    if isempty(Vend)
        Vend = length(valid_range);
    elseif length(Vstart) > length(Vend)
        Vend = [Vend length(valid_range)]; %#ok<AGROW>
    end
    Vstart = Vstart + m1 - 1;
    Vend = Vend + m1 - 1;
    ...
    S_tmp = hit_table2 ./ ( num_hitsperchan * ones(1,num_chans) );
    S_tmp(1:size(S_tmp,1)+1:end) = 0;
    S_matrix(:,:,i_count) = S_tmp;
end
if option ~= 's'
    fprintf('done\n');
end
...
if exist('settings','var') & isfield(settings,'SL') & ...
        isfield(settings.SL,'storesingle') & settings.SL.storesingle == true
