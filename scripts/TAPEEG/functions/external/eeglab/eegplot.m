...
if ~isstr(data) % If NOT a 'noui' call or a callback from uicontrols
  ...
  defctrlupcom = ''; % CTRL press and up -> do nothing by default
   ...
   DEFAULT_AXES_POSITION = [0.065 0.11 0.89 0.92-(MAXEVENTSTRING-5)/100];
%    DEFAULT_AXES_POSITION = [0.0964286 0.15 0.842 0.75-(MAXEVENTSTRING-5)/100];
...
% positions of buttons
  ...
  posbut(20,:) = [-0.19   0.15     0.015    0.8 ]; % slider
  posbut(21,:) = [0.76    0.935    0.04      0.03]; % normalize
  posbut(22,:) = [0.76    0.97    0.04      0.03]; % stack channels(same offset)
...
%           uimenu('Parent',m(11),'Label','Edit bad channels','Callback', ...
%   	        ['g = get(gcbf, ''userdata'');' ...
%             'set(gcbf, ''userdata'', g);' ...
%             'eegplot(''drawb''); clear g;'] )
...
%        keyboard;  
  ...
  case 'drawb' % Draw background ******************************************************
    ...
    if strcmpi(g.plotevent, 'on')
        ...
        AXES_POSITION = [0.065 0.11 0.89 0.92-(MAXEVENTSTRING-5)/100];
